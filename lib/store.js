    ...
    if (criterion !== undefined) {
        ...
        if(Array.isArray(criterion))
        {
            ...
            for (let i = 0; i < criterion.length; ++i) {
                ...
                if (node.$range) {
                    //ADD RANGE LOGIC HERE
                }
                else if (node[criterion[i]] !== undefined) {
                    exports._logApplied(applied, filter, node, criterion[i]);
                    arraynode[i] = internals.defaults(internals.filter(node[criterion[i]], criteria[i], applied), node.$base);
                }
            }
            return arraynode;
        }
